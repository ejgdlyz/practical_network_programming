...
static thread_local int t_numOpenedFiles = 0;
...
static thread_local std::vector<pid_t>* t_pids = NULL;
...
sylar::Timestamp g_startTime = sylar::Timestamp::now();
...
sylar::Timestamp ProcessInfo::startTime(){
    return g_startTime;
}
...
// std::string ProcessInfo::procname(const std::string& stat) {
//     size_t lp = stat.find('(');
//     size_t rp = stat.rfind(')');
//     if (lp != std::string::npos && rp != std::string::npos && lp < rp) {
//         return stat.substr(lp + 1, static_cast<int>(rp - lp - 1));
//     }
//     return "";
// }
