...
std::string base64decode(const std::string& src) {
    ...
    while (ptr < end) {
        int i = 0, padding = 0, packed = 0;
        // 取出 4 组 6 bit 数据(即每个字符在索引表中索引)放入一个 整型 int 即 packed 中，
        // 每字节 6 个有效 bit，共 24 bit，将其还原为 3 组
        for (; i < 4 && ptr < end; ++i, ++ptr) {
            ...
            packed = (packed << 6) | val;   // 按 6 bit 取数据
        }
        if (i != 4) {
            return "";
        }
...
std::string sha1sum(const void* data, size_t len) {
    ...
    std::string result;
    result.resize(SHA_DIGEST_LENGTH);  // 160 字节
