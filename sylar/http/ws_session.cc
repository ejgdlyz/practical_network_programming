...
HttpRequest::ptr WSSession::handleShake() {
    ...
    do {
        ...
        if (req->getHeaderAs<int>("Sec-WebSocket-Version") != 13) {
            SYLAR_LOG_INFO(g_logger) << "http header Sec-WebSocket-Version != Upgrade";
            break;
        }
        std::string key = req->getHeader("Sec-WebSocket-Key");
        if (key.empty()) {
            SYLAR_LOG_INFO(g_logger) << "http header Sec-WebSocket-Key is null";
            break;
        }
...
int32_t WSSendMessage(Stream* stream, WSFrameMessage::ptr msg, bool client, bool fin) {
    do {
        ...
        ws_head.mask = client;  // 客户端的发送的数据帧必须使用 mask
        ...
        if(client) {
            ...
            std::string& data = msg->getData();     // 引用, 将修改 msg 的内容
            for(size_t i = 0; i < data.size(); ++i) {
                data[i] ^= mask[i % 4];             // 使用 mask 对数据进行掩码处理
            }
