...
%%{
    ...
    action save_scheme
    {
        uri->setScheme(std::string(mark, fpc - mark));  /** 设置 scheme **/
        mark = NULL;
    }
    ...
    action save_userinfo
    {
        if(mark) {
            ...
            uri->setUserinfo(std::string(mark, fpc - mark));  /** 设置 userinfo **/
        }
        mark = NULL;
    }
    action save_host
    {
        if (mark != NULL) {
            //std::cout << std::string(mark, fpc - mark) << std::endl;
            uri->setHost(std::string(mark, fpc - mark));  /** 设置 host **/
        }
    }
    ...
    {
            //std::cout << std::string(mark, fpc - mark) << std::endl;
        uri->setPath(std::string(mark, fpc - mark));  /** 设置 path **/
        mark = NULL;
    }
...
# add (any -- ascii) support chinese
    ...
    {
        //std::cout << std::string(mark, fpc - mark) << std::endl;
        uri->setQuery(std::string(mark, fpc - mark));  /** 设置 query **/
        mark = NULL;
    }
    action save_fragment
    {
        //std::cout << std::string(mark, fpc - mark) << std::endl;
        uri->setFragment(std::string(mark, fpc - mark));  /** 设置 fragment **/
        mark = NULL;
    }
...
Uri::ptr Uri::Create(const std::string& uri_str) {
    ...
    const char *mark = 0;
    %% write init;
    const char *p = uri_str.c_str();  /** 首指针 **/
    const char *pe = p + uri_str.size();  /** 尾指针 **/
    const char* eof = pe;       /** 定义参数 eof **/
    %% write exec;
    if (cs == uri_parser_error) {
        return nullptr;
    } else if (cs >= uri_parser_first_final) {  /** 成功解析 **/
        return uri;
    }
    return nullptr;
}
