...
static T CreateMask(uint32_t bits) {
    return (1 << (sizeof(T) * 8 - bits)) - 1;  // 将低 32 - bits 位设置为 1，即主机位全1
}
...
bool Address::Lookup(std::vector<Address::ptr>& result, const std::string& host, 
    ...
    if (!host.empty() && host[0] == '[') {
        ...
        if (endipv6) {
            ...
            if (*(endipv6 + 1) == ':') {
                service = endipv6 + 2;  // 前移，去掉端口号后是 service
            }
            node = host.substr(1, endipv6 - host.c_str() - 1);  // ipv6 地址
        }
    }
...
bool Address::GetInterfaceAddresses(std::vector<std::pair<Address::ptr, uint32_t>>& result, 
    ...
    if (iface.empty() || iface == "*") {
        if (family == AF_INET || family == AF_UNSPEC) {
            result.push_back(std::make_pair(Address::ptr(new IPv4Address()), 0u));  // 任意地址
        }
        if (family == AF_INET6 || family == AF_UNSPEC) {
            result.push_back(std::make_pair(Address::ptr(new IPv6Address()), 0u));  // 任意地址
        }
        return true;
    }
...
IPv4Address::IPv4Address(uint32_t address, uint16_t port) {
    ...
    m_addr.sin_port = byteswapOnLittleEndian(port);  // 转为网络字节序
...
IPv6Address::IPv6Address(const uint8_t address[16], uint16_t port) {
    memset(&m_addr, 0, sizeof(m_addr));
    m_addr.sin6_family = AF_INET6;
    m_addr.sin6_port = byteswapOnLittleEndian(port);  // 转为网络字节序
...
IPAddress::ptr IPv6Address::subnetMask(uint32_t prefix_len) {
    sockaddr_in6 subnet;
    memset(&subnet, 0, sizeof(subnet));
    subnet.sin6_addr.s6_addr[prefix_len / 8] = ~CreateMask<uint32_t>(prefix_len % 8);
