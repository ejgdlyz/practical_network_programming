...
void Socket::shutdownWrite() {
    ...
    if (::shutdown(m_sock, SHUT_WR)) {
        SYLAR_LOG_ERROR(g_logger) << "shutdown(" << m_sock << ") errno"
...
bool Socket::bind(const Address::ptr addr) {
    ...
    if (::bind(m_sock, addr->getAddr(), addr->getAddrLen())) {
        SYLAR_LOG_ERROR(g_logger) << "bind error errno="
                << errno << " errstr" << strerror(errno);
        return false;
    }
    getLocalAddress();
...
bool Socket::connect(const Address::ptr addr, uint64_t timeout_ms) {
    ...
    if (timeout_ms == (uint64_t)-1) {
        ...
        if (::connect_with_timeout(m_sock, addr->getAddr(), addr->getAddrLen(), timeout_ms)) {
            SYLAR_LOG_ERROR(g_logger) << "sock=" << m_sock << " connect_with_timeout(" << addr->toString()
...
void Socket::initSock() {
    int val = 1;
    setOption(SOL_SOCKET, SO_REUSEADDR, val);  // 重用本地地址
    if (m_type == SOCK_STREAM) {
        setOption(IPPROTO_TCP, TCP_NODELAY, val);  // 禁止 Nagle 算法
    }
}
